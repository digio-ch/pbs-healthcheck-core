...
class WidgetLeaderOverviewRepository extends AggregatedEntityRepository
{
    /**
     * WidgetLeaderOverviewRepository constructor.
    ...
    public function findMaleFemaleMembersCountForGroupTypeAndDate(int $mainGroupId, array $groupTypes, string $date)
    {
        ...
        $statement = $conn->executeQuery(
            "SELECT SUM(m_count) as m, SUM(f_count) as f, SUM(u_count) as u
                    FROM hc_widget_leader_overview
                    WHERE hc_widget_leader_overview.data_point_date = ? AND 
                          hc_widget_leader_overview.group_id = ? AND
                          hc_widget_leader_overview.group_type IN (?);",
